...
let course='easy',phase='home',run=null,timer=null,rankRequest=0;
...
$('courses').onclick=e=>{const b=e.target.closest('[data-course]');if(!b)return;course=b.dataset.course;const sample={easy:[72,4],normal:[127,7],hard:[237,12]}[course];$('previewCurrent').textContent=sample[0];$('previewMission').textContent=`연속으로 ${sample[1]}씩 빼세요`;document.querySelectorAll('[data-course]').forEach(el=>{const active=el===b;el.classList.toggle('chosen',active);el.setAttribute('aria-pressed',active);});};
...
function refresh(){$('current').textContent=run.correct?run.expected[run.correct-1]:run.start;$('count').textContent=`${run.correct} / ${run.expected.length}개 완료`;$('progressFill').style.width=100*run.correct/run.expected.length+'%';inputDisplay();}
...
show('play');$('timer').textContent='0.0초';$('modeLabel').textContent=params.name;$('mission').textContent=`연속으로 ${run.step}씩 빼세요`;$('feedback').textContent='';$('keypad').inert=true;
const readyAt=performance.now()+3000;run.started=readyAt;refresh();$('current').textContent='3';clearInterval(timer);
timer=setInterval(()=>{if(phase!=='play')return;const now=performance.now();if(now<readyAt){$('current').textContent=Math.ceil((readyAt-now)/1000);return;}if(!run.ready){run.ready=true;$('keypad').inert=false;refresh();}$('timer').textContent=((now-readyAt)/1000).toFixed(1)+'초';},50);$('start').disabled=false;}
$('start').onclick=start;
function key(k){
 if(phase!=='play'||!run.ready||$('quitDialog').open)return;
 if(!/^\d$/.test(k)&&!['back','clear'].includes(k))return;
 const wasRejected=run.rejected;
...
function submit(){
 ...
 if(n===run.expected[run.correct]){run.correct++;run.input='';run.rejected=false;$('feedback').textContent='';}
 else{run.rejected=true;$('feedback').textContent='오답입니다. 다음 숫자부터 다시 입력하세요.';}
 refresh();
 if(run.rejected){void $('answer').offsetWidth;$('answer').classList.add('shake');}
...
async function finish(){clearInterval(timer);run.ready=false;run.elapsed=Math.round(performance.now()-run.started);const finished=run;show('result');$('resultCourse').textContent=run.label;$('elapsed').textContent=format(run.elapsed/run.correct);$('correctStat').textContent=run.correct+'개';$('publishBox').hidden=true;$('publishStatus').textContent='';$('consent').checked=false;$('publish').disabled=false;
...
if(run.remote){$('saveStatus').textContent+=' 랭킹용 기록을 확인하고 있습니다…';try{const verified=await api('/finish',{token:run.remote.token,identity,attempts:run.attempts});finished.verified=verified;if(run===finished&&phase==='result'){$('publishBox').hidden=false;$('nickname').value=storage.get('training.nickname','');$('saveStatus').textContent=`개인 기록 ${format(finished.elapsed/finished.correct)}초/문항 · 랭킹 기록 ${format(verified.average)}초/문항 (서버 측정·통신 시간 포함)`;}}catch(e){if(run===finished&&phase==='result')$('saveStatus').textContent=`개인 기록은 ${saved?'저장했습니다':'저장하지 못했습니다'}. 랭킹 기록 확인 실패: ${e.message}`;}}
}
$('publish').onclick=async()=>{if(!run?.verified||run.published)return;const nickname=$('nickname').value.trim();if(!validNickname(nickname)){$('publishStatus').textContent='닉네임은 한글·영문·숫자·_로 2~12자 입력해 주세요.';return;}if(!$('consent').checked){$('publishStatus').textContent='기록 공개에 동의해 주세요.';return;}const finished=run;$('publish').disabled=true;try{await api('/publish',{token:finished.remote.token,identity,nickname});finished.published=true;storage.set('training.nickname',nickname);if(run===finished)$('publishStatus').textContent='등록했습니다. 난이도·빼는 수별 최고 기록이 저장됩니다.';}catch(e){if(run===finished){$('publishStatus').textContent=e.message;$('publish').disabled=false;}}};
