import {requestTrainingJSON} from './network.mjs?v=20260911-plot1';
import {createScreenHistory} from './navigation.mjs?v=20260911-plot1';
import {createRankingExplorer} from './analytics.mjs?v=20260911-plot1';
import {COURSES,sequence,assess,validNickname,randomProblem,courseSteps,formatKST,bindPressInput,bindHubShare} from './core.mjs?v=20260911-plot1';
...
let rankNickname='';
async function ranking(){const serial=++rankRequest;show('ranking');if(explorer.active()){await explorer.render();return;}explorer.invalidate();explorer.render();const search=rankNickname;$('rankNote').textContent=search?'검색 결과는 최대 50건이며, 순위는 선택한 코스·빼는 수 전체 기록 기준입니다.':'상위 50위까지만 조회할 수 있습니다';show('ranking');const list=$('rankList');if(!apiBase){empty(list,'전체 랭킹은 연결 준비 중입니다. 현재는 나의 기록에서 개인 훈련 결과를 확인할 수 있습니다.');return;}empty(list,'기록을 불러오고 있습니다…');try{const {rows}=await api('/ranking?course='+$('rankCourse').value+($('rankStep').value==='all'?'':'&step='+$('rankStep').value)+($('rankPeriod').value==='week'?'&period=week':'')+(search?'&nickname='+encodeURIComponent(search):''));if(serial!==rankRequest)return;list.replaceChildren();if(!rows.length)empty(list,search?'선택한 조건에 해당 닉네임의 기록이 없습니다.':'아직 등록된 기록이 없습니다. 첫 기록의 주인공이 되어 보세요.');rows.forEach((r,i)=>list.append(row(`${r.rank??i+1}. ${r.nickname}`,`${r.step}씩 빼기 · ${COURSES[r.course]?.name||''} · 시작 ${r.start}`,format(r.average)+'초/문항',formatKST(r.created))));}catch(e){if(serial===rankRequest)empty(list,'랭킹을 불러오지 못했습니다. '+e.message);}}
$('showRanking').onclick=enterRanking;$('rankCourse').onchange=()=>{rankOptions();ranking();};$('rankStep').onchange=ranking;document.querySelectorAll('.back').forEach(b=>b.onclick=()=>{rankRequest++;show('home');homeNote();});homeNote();
...
function enterRanking(){$('rankCourse').value='all';rankOptions();$('rankStep').value='all';rankNickname='';$('rankPeriod').value='all';$('rankNickname').value='';explorer.reset();return ranking();}
